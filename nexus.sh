...
if [ -d "/workspaces/Jambu/Nexus/network-api" ]; then
    show "Deleting existing repository..." "progress"
    sudo rm -rf "/workspaces/Jambu/Nexus/network-api"
fi

sleep 2

show "Cloning Nexus-XYZ network API repository..." "progress"
if ! git clone https://github.com/nexus-xyz/network-api.git "/workspaces/Jambu/Nexus"; then
    show "Failed to clone the repository." "error"
    exit 1
fi

cd /workspaces/Jambu/Nexus/network-api/clients/cli
...
[Service]
User=$USER
WorkingDirectory=/workspaces/Jambu/Nexus/network-api/clients/cli
Environment=NONINTERACTIVE=1
ExecStart=/workspaces/Jambu/Nexus/.cargo/bin/cargo run --release --bin prover -- beta.orchestrator.nexus.xyz
Restart=always
RestartSec=10
